class Converter:
    ...
    def is_normalized_binary(func):
        ...
        def wrapper(binary_str, *args):
            ...
            if not isinstance(binary_str, str): binary_str = str(binary_str)
    ...
    def is_normalized_decimal(func):
        ...
        def wrapper(decimal_str, *args):
            # Check if input is a string
            if not isinstance(decimal_str, str): decimal_str = str(decimal_str)
    ...
    def dec_to_sign_magnitude(dec: str, bits=32) -> str:
        ...
        num = Converter.dec_to_bin(abs(dec), bits-1)
        return f"{msb}{num}"
    ...
    def ones_complement_to_dec(bin: str) -> str:
        msb = bin[0]
        sign = "-" if msb == "1" else "(+)"
        if msb == "1": new_bin = "".join(str(1-int(i)) for i in bin)
        num = Converter.bin_to_dec(new_bin) if msb == "1" else Converter.bin_to_dec(bin)
        return f"{sign}{num}"

    @staticmethod
    @is_normalized_decimal
    def dec_to_ones_complement(dec: str, bits=32) -> str:
        dec = int(dec)
        msb = "1" if dec < 0 else "0"
        num = Converter.dec_to_bin(abs(dec), bits-1)
        if msb == "1": num = "".join(str(1-int(i)) for i in num)
        return f"{msb}{num}"
    ...
    def fixed_point_to_dec(bin: str, k: int, j: int) -> str:
        if len(bin) != k + j + 1: return f"Binary number has a length of {len(bin)} but should have a length of {k+j+1}"
    ...
    @staticmethod
    @is_normalized_float
    def dec_to_fixed_point(dec: str, k: int, j: int) -> str:
        dec = float(dec)
        if not (k > 0 and isinstance(k, int)) or not (j > 0 and isinstance(j, int)): return "`k` and `j` must be integers and can't be 0 or negative."
        int_part: str = Converter.dec_to_sign_magnitude(int(dec), k+1)
        ...
        while True:
            ...
            comma_value += str(num)
            counter+=1
        if len(comma_value) < j: comma_value = Converter.fill_up_bits(comma_value, j)
    ...
    def ieee_to_dec(self, bin: str, bits=32) -> str:
        ...
        """
        if not bits in [32, 64]: return "IEEE 754 needs to have 32 or 64Bit"
        if len(bin) != bits: return f"The provided binary has a size of {len(bin)} Bit but {bits} are needed."
    ...
    @is_normalized_float
    def dec_to_ieee(self, dec: str, bits=32) -> str:
        dec = float(dec)
        if not bits in [32, 64]: return "IEEE 754 needs to have 32 or 64Bit"
        ...
        charakteristik = Converter.dec_to_bin(E+B, C)
...
if __name__ == "__main__":
    ...
    parser.add_argument("number",
                        type=str,
                        help="The number that should be converted.")
